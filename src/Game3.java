...
public class Game3 extends Applet implements ActionListener, ItemListener {
...
class Chess_Board extends Canvas implements MouseListener {
	...
	//constructor
	public Chess_Board (Image k, Game3 s) {
	...
	public void paint(Graphics g) {
		...
		if (dimension == 0) {             //checking if the game is over for the 5x5 board
			for (int i =0; i < 64; i++) {
				if ((i%8 < 5) && (i/8 < 5)) {
					if (squares[i] == 3) {
						game_over = false;
		...
		} else if (dimension == 1) {             //checking if the game is over for the 6x6 board
			for (int i =0; i < 64; i++) {
				if ((i%8 < 6) && (i/8 < 6)) {
					if (squares[i] == 3) {
						game_over = false;
		...
		} else if (dimension == 2) {             //checking if the game is over for the 7x7 board
			for (int i = 0; i < 64; i++) {
				if ((i%8 < 7) && (i/8 < 7)) {
					if (squares[i] == 3) {
						game_over = false;
		...
		if (game_over) {                         //sets applet to game over screen
			...
			if ((dimension == 3 && score == 64) || 
				(dimension == 2 && score == 49) ||
				(dimension == 1 && score == 36) ||
				(dimension == 0 && score == 25)){ //checking to see if user has won
	...
	public void fill_grid (Graphics g, int height, int width) {
		//draws the grid
		int max = 0;
		if (dimension == 3) max = 7;
		else if (dimension == 2) max = 6;
		else if (dimension == 1) max = 5;
		else max = 4;
		
		for (int j = 0; j <= max; j++) {  //this loop draws squares vertically

			for (int i = 0; i <= max; i++) {   //this loop draws squares horizontally
				if (squares[j*8 + i] == 0)
					g.setColor(Color.white);
	...
	public void mousePressed(MouseEvent e) {
		...
		for (int i=0; i <= 7; i++)
			if (dimension == 3) {
				...
				if (index == final_array[i] && (final_array[i]%8) < 7 && (final_array[i]/8) < 7) {
					contains = true;
					break;
				}
			} else if (dimension == 1) {
				if (index == final_array[i] && (final_array[i]%8) < 6 && (final_array[i]/8) < 6) {
					contains = true;
					break;
				}
			} else {
	...
	//method determines whether board is 8x8 or 5x5
		...
		int size_choice = s;
		if (size_choice == 0) dimension = 3;
		else if(size_choice == 1) dimension = 2;
		else if(size_choice == 2) dimension = 1;
		else dimension = 0;
		repaint();
	}
